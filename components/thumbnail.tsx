...
export const Thumbnail = ({ src, fallback, isLive, username }: ThumbnailProps) => {
  ...
  if (!src) {
    content = (
      <div className="bg-background flex flex-col justify-center items-center gap-y-4 h-full w-full transition-transform group-hover:translate-x-2 group-hover:-translate-y-2 rounded-md">
        <UserAvatar size="lg" showBadge isLive={isLive} username={username} imageUrl={fallback} />
      </div>
    );
  } else {
    content = (
